...
-->
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ns0="bioagents schema" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/xpath-functions" exclude-result-prefixes="ns0 xs fn">
	<xsl:output method="xml" encoding="UTF-8" byte-order-mark="no" indent="yes"/>
	<xsl:template match="/">
		<root>
			<xsl:attribute name="xsi:noNamespaceSchemaLocation" namespace="http://www.w3.org/2001/XMLSchema-instance" select="'file:///E:/repos/GitHub/bioagentsShim/genericxml2xml-singleagent/versions/bioagents-3.3.0/bioagents-3.3.0_framework_singleagent.xsd'"/>
			<xsl:for-each select="ns0:agents/ns0:agent">
				...
				<xsl:for-each select="ns0:bioagentsID">
					<bioagentsID>
						<xsl:sequence select="xs:string(xs:anyURI(fn:string(.)))"/>
					</bioagentsID>
				</xsl:for-each>
				<xsl:for-each select="ns0:bioagentsCURIE">
					<bioagentsCURIE>
						<xsl:sequence select="xs:string(xs:anyURI(fn:string(.)))"/>
					</bioagentsCURIE>
				</xsl:for-each>
				...
				<agentType>
					<xsl:for-each select="ns0:agentType">
						<list-item>
							<xsl:sequence select="fn:string(.)"/>
						</list-item>
					</xsl:for-each>
				...
				<iechorPlatform>
					<xsl:for-each select="ns0:iechorPlatform">
						<list-item>
							<xsl:sequence select="fn:string(.)"/>
						</list-item>
					</xsl:for-each>
				...
				<iechorCommunity>
					<xsl:for-each select="ns0:iechorCommunity">
						<list-item>
							<xsl:sequence select="fn:string(.)"/>
						</list-item>
					</xsl:for-each>
				...
				<iechorNode>
					<xsl:for-each select="ns0:iechorNode">
						<list-item>
							<xsl:sequence select="fn:string(.)"/>
						</list-item>
					</xsl:for-each>
