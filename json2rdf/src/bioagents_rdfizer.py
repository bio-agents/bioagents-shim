...
BioAgents RDFizer agent, v0.1a, for testing purpose only. 

Sample Usage : 
    python bioagents_rdfizer.py [--proxy_url <PROXY_URL>] --agent_id signalp 
    python bioagents_rdfizer.py [--proxy_url <PROXY_URL>] --test_dump 
    python bioagents_rdfizer.py [--proxy_url <PROXY_URL>] --dump 
...
parser.add_argument('-id', '--agent_id', type=str, help='the ID of the Bise resource to be exported', dest='id', required=False)
parser.add_argument('-dump', '--dump', help='complete Bio.Agents RDF dump', dest='dump', action='store_true', required=False)
parser.add_argument('-test', '--test_dump', help='dump only 10 Bio.Agents resources in RDF', dest='test', action='store_true', required=False)

def main():
    args = parser.parse_args()

    if (args.id is None) and (args.dump is False) and (args.test is False):
        print('Please fill one of the --agent_id, --dump, or --test_dump parameters')
        print()
        parser.print_help()
        exit(0)
    ...
    if args.id:
        ...
        # bioagentsID = "signalp"
        # bioagentsID = "bwa"
        raw_jld = get_agent_as_linked_data(args.id, connection)
        import_to_graph(graph, raw_jld)
        print(str(graph.serialize(format='turtle').decode('utf-8')))

    if args.test:
        graph = Graph()
        crawl_agents_as_linked_data(graph, connection, limit=100)
        graph.serialize(destination='bioagents-dump-' + time.strftime("%Y%m%d") + '.ttl', format='turtle',
                        encoding='utf-8')

    if args.dump:
        graph = Graph()
        # -1 value as limit to dump all content
        crawl_agents_as_linked_data(graph, connection, limit=-1)
        graph.serialize(destination='bioagents-dump-' + time.strftime("%Y%m%d") + '.ttl', format='turtle',
                        encoding='utf-8')
    ...
    try:
        ...
        ctx = {
            "@context": {
                ...
                "agentType": "datacite:resourceType",
                ...
                "hasOperation": "bioagents:has_function",
                "hasInputData": "edam:has_input",
                "hasOutputData": "edam:has_output",
                "hasTopic": "edam:has_topic"
            }
        }
        entry['@id'] = str(entry['bioagentsID'])
        entry['@type'] = {"@id": 'bioagents:Resource'}
        entry.update(ctx)
...
def get_agent_as_linked_data(agent_id, connection):
    """
    Given its identifier, transforms a bioagent entry in RDF using common vocabularies.
    :param agent_id: the bioagent identifier
    ...
    try:
        req = http.request('GET', 'https://bio.agents/api/agent/' + str(agent_id) + '?format=json')
...
def crawl_agents_as_linked_data(graph, connection, limit):
    """
    transforms a bioagent entry in RDF using common vocabularies.
    ...
    http = get_web_service(connection)
    try:
        req = http.request('GET', 'https://bio.agents/api/agent/?page=1&?format=json')
        countJson = json.loads(req.data.decode('utf-8'))
        count = int(countJson['count'])
        print(str(count)+ " available BioAgents entries")

        i = 1
        nbAgents = 1
        hasNextPage = True
        while hasNextPage :
            req = http.request('GET', 'https://bio.agents/api/agent/?page=' + str(i) + '&?format=json')
            try:
                entry = json.loads(req.data.decode('utf-8'))
            ...
            for agent in entry['list']:
                ...
                nbAgents += 1
                progress = nbAgents * 100 / count
                if (nbAgents % 500 == 0) :
                    print(str(round(progress))+" % done")
                if ((limit != -1) and (nbAgents >= limit)):
                    return
            i+=1
